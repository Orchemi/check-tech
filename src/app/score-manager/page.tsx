...
export default function ScoreManagerPage() {
  ...
  const handleDeleteLog = useCallback((logId: string) => {
    setScoreData((prev) => {
      const logToDelete = prev.logs.find((log) => log.id === logId);
      if (!logToDelete) return prev;

      const newScores = { ...prev.scores };
      newScores[logToDelete.person] =
        (newScores[logToDelete.person] || 0) - logToDelete.score;
  ...
  const handleResetAll = useCallback(() => {
    if (
      !confirm(
        '모든 점수와 로그를 초기화하시겠습니까? 이 작업은 되돌릴 수 없습니다.',
      )
    ) {
      return;
    }
